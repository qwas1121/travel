...
폴더는 3단계입니다: **최상위(사이트 전체) → 여행 → 앨범 → 사진**

1. Google Drive에서 최상위 폴더를 하나 만듭니다. 예: `일송세희 여행`
2. 그 안에 여행별로 하위 폴더를 만듭니다. 예: `유럽 신혼여행`, `LA`. 나중에 여행을 추가하고 싶으면 여기에 폴더만 새로 만들면 됩니다 (코드 수정 불필요).
3. 각 여행 폴더 안에 다시 앨범별 하위 폴더를 만듭니다. **폴더 이름 앞에 숫자를 붙이면 그 순서대로 화면에 표시됩니다.**
   ```
   일송세희 여행/
   ├── 유럽 신혼여행/
   │   ├── 01_인천 to 로마/
   ...
   └── LA/
       └── 01_첫날/
   ```
4. 각 앨범 폴더 안에 사진 파일을 올립니다. 파일 이름 순서대로 정렬되니, 순서가 중요하면 파일명 앞에 `001_`, `002_` 처럼 번호를 붙여주세요.
5. 최상위 폴더(`일송세희 여행`)를 열고 주소창의 URL을 확인하세요.
   ```
   https://drive.google.com/drive/folders/여기가_폴더_ID
   ```
   이 `여기가_폴더_ID` 부분을 복사해두세요. → `GOOGLE_DRIVE_ROOT_FOLDER_ID` (최상위 폴더만 서비스 계정과 공유하면, 그 안의 여행/앨범 폴더는 자동으로 다 읽힙니다)
...
9. 다시 Google Drive로 돌아가서, 1단계에서 만든 최상위 폴더(`일송세희 여행`)를 **우클릭 → 공유** → `client_email` 주소를 붙여넣고 **뷰어(Viewer)** 권한으로 공유합니다. (이 단계를 빼먹으면 사진을 못 읽어옵니다!)

---

## 3. Supabase 설정 (여행/앨범 제목 꾸미기는 선택, 댓글 기능은 필수)

Supabase 없이도 앱은 잘 동작합니다 (여행/앨범 제목이 폴더명 그대로 표시됨). 다만 **사진에 댓글 남기는 기능은 Supabase가 있어야 동작**해요. 나중에 추가해도 됩니다.
...
여행/앨범 제목을 바꾸고 싶다면 Supabase의 **Table Editor → trips** 또는 **albums**에서 `drive_folder_id`(Drive 폴더 URL의 ID)를 채운 행을 추가하고 `title`, `description`을 원하는 대로 입력하면 됩니다. 댓글은 앱에서 직접 남기면 `photo_comments` 테이블에 자동으로 쌓입니다.
...
- [ ] 올바른 비밀번호 입력 후 여행 목록이 보이는지
- [ ] 로그아웃 후 다시 `/trip/아무거나` 주소로 직접 들어가면 로그인 화면으로 돌아가는지
- [ ] 여행을 클릭하면 앨범 목록(폴더별/날짜별 전환 포함)이 보이는지
- [ ] 앨범을 클릭하면 사진 그리드가 보이는지
- [ ] 사진을 클릭하면 원본 크기로 크게 보이는지 (좌우 화살표/ESC로 이동·닫기)
- [ ] 사진 오른쪽 아래 💬 버튼을 누르면 댓글을 남길 수 있는지 (Supabase 설정 후)

## 사진 추가하는 방법 (평소 사용법)

Google Drive의 해당 앨범 폴더에 사진을 올리기만 하면 됩니다. 새 여행을 시작하려면 최상위 폴더에 새 여행 폴더를 만들고 그 안에 앨범 폴더를 만들면 됩니다. 사이트를 새로고침하면 바로 반영됩니다 (별도 배포/코드 수정 불필요).
